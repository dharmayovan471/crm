...
import { TenantRegisterDto, TenantLoginDto, UpdateTenantDto } from '../dto/tenant.dto';
import { NodePgDatabase } from 'drizzle-orm/node-postgres';
import { eq, inArray, sql } from 'drizzle-orm';
import { roles, userRoles, rolePermissions, permissions } from '../../database/schemas/tenant.schema';
...
export class TenantService {
  ...
  async register(dto: TenantRegisterDto) {
    // 1. Check if tenant code or email already exists
    const existingTenant = await this.tenantRepository.findTenantByCode(dto.tenantCode);
    if (existingTenant) {
      throw new ConflictException(`Tenant with code '${dto.tenantCode}' already exists`);
    }

    const existingEmail = await this.tenantRepository.findTenantByEmail(dto.email);
    if (existingEmail) {
      throw new ConflictException(`Tenant contact email '${dto.email}' is already registered`);
    }

    const schemaName = `tenant_${dto.tenantCode.toLowerCase().replace(/[^a-z0-9_]/g, '')}`;

    // 2. Create the tenant record in the public schema
    ...
    try {
      // 3. Create PostgreSQL schema dynamically using SQL: CREATE SCHEMA IF NOT EXISTS tenant_xxx;
      await this.publicDb.execute(sql.raw(`CREATE SCHEMA IF NOT EXISTS "${schemaName}"`));

      // 4. Create tenant metadata tables first
      ...
      await this.publicDb.execute(sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schemaName}"."users" (
          ...
          "designation_id" uuid REFERENCES "${schemaName}"."designations"("id") ON DELETE SET NULL,
      ...
      await this.publicDb.execute(sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schemaName}"."quotations" (
          ...
          "revision_no" varchar(50) DEFAULT 'R0' NOT NULL,
          ...
          "terms" text,
          "notes" text,
          "prepared_by" uuid REFERENCES "${schemaName}"."users"("id") ON DELETE SET NULL,
          "approved_by" uuid REFERENCES "${schemaName}"."users"("id") ON DELETE SET NULL,
          "signature" text,
          "created_at" timestamp DEFAULT now() NOT NULL
        );
      `));
      ...
      // Seed default lead number settings
      await this.publicDb.execute(sql.raw(`
      ...
      // 6. Create default admin role with all permissions
      const roleResult = await this.publicDb.execute(sql`
        INSERT INTO ${sql.raw(`"${schemaName}"."roles"`)} ("name", "permissions")
        VALUES ('admin', ARRAY['*'])
      ...
      // 7. Associate all permissions with admin role in role_permissions table
      const permissionsResult = await this.publicDb.execute(sql`
        SELECT "id" FROM ${sql.raw(`"${schemaName}"."permissions"`)}
      `);
      for (const row of permissionsResult.rows) {
        await this.publicDb.execute(sql`
          INSERT INTO ${sql.raw(`"${schemaName}"."role_permissions"`)} ("role_id", "permission_id")
      ...
      const userResult = await this.publicDb.execute(sql`
        INSERT INTO ${sql.raw(`"${schemaName}"."users"`)} ("email", "password", "role_id")
        VALUES (${adminEmail}, ${hashedPassword}, ${roleId})
        ON CONFLICT ("email") DO NOTHING
        RETURNING "id"
      `);
      const userId = userResult.rows[0].id as string;

      // 9. Associate user with role in user_roles table
      await this.publicDb.execute(sql`
        INSERT INTO ${sql.raw(`"${schemaName}"."user_roles"`)} ("user_id", "role_id")
        VALUES (${userId}, ${roleId})
        ON CONFLICT DO NOTHING
      `);

      // 7. Seed initial subscription plan and link it
      let plan = await this.tenantRepository.findPlanByCode('standard');
      if (!plan) {
        // Automatically create a default standard plan if none exists
        const planResult = await this.publicDb.execute(sql`
          INSERT INTO "plans" ("name", "code", "description", "price", "billing_cycle")
          VALUES ('Standard Plan', 'standard', 'Default standard subscription plan', '99.00', 'monthly')
          RETURNING "id"
        `);
        plan = { id: planResult.rows[0].id } as any;
      }
      ...
      // In a production app, rollback/delete tenant if schema creation fails
      await this.publicDb.execute(sql`DELETE FROM "tenants" WHERE "id" = ${tenant.id}`);
      throw error;
    }
  }

  async login(dto: TenantLoginDto) {
    // 1. Find tenant in public database
    const tenant = await this.tenantRepository.findTenantByCode(dto.tenantCode);
    if (!tenant) {
      throw new UnauthorizedException('Invalid tenant code or credentials');
    }

    if (tenant.status !== 'active') {
      throw new UnauthorizedException('Tenant is inactive');
    }
    // 2. Find user in the tenant's schema
    // Note: The TenantDatabaseInterceptor has already switched search_path to the resolved tenant schema
    const userWithRole = await this.tenantRepository.findUserByEmail(dto.email);
    if (!userWithRole) {
      throw new UnauthorizedException('Invalid tenant code or credentials');
    }

    // 3. Verify bcrypt password
    const isPasswordValid = await this.authService.comparePassword(dto.password, userWithRole.password);
    if (!isPasswordValid) {
      throw new UnauthorizedException('Invalid tenant code or credentials');
    }

    // 4. Resolve Roles and Permissions for the user (including legacy fallbacks)
    ...
    const rolesResult = await db
      ...
      .where(eq(userRoles.userId, userWithRole.id));

    let rolesList = rolesResult.map((r: any) => r.name as string);
    ...
    if (rolesResult.length > 0) {
      ...
      permsList = permsResult.map((p: any) => p.code as string);

      // Check if any role has wildcard permission '*' in legacy column
      const rolesWithWildcard = await db
        .select({ name: roles.name, permissions: roles.permissions })
        .from(roles)
        .where(inArray(roles.id, rolesResult.map((r: any) => r.id)));
      for (const r of rolesWithWildcard) {
        if (r.permissions && r.permissions.includes('*')) {
          permsList.push('*');
        }
      }
    } else if (userWithRole.roleId) {
      // Fallback to legacy role mapping
      const legacyRoleResult = await db
        .select({
          name: roles.name,
          permissions: roles.permissions,
        })
        .from(roles)
        .where(eq(roles.id, userWithRole.roleId))
        .limit(1);

      if (legacyRoleResult[0]) {
        rolesList = [legacyRoleResult[0].name];
        permsList = legacyRoleResult[0].permissions || [];
      }
    }
    ...
    const tokens = await this.authService.generateTokens({
      ...
      tenantCode: tenant.tenantCode,
      schemaName: tenant.schemaName,
      roles: rolesList.map((r) => r.toUpperCase()),
  ...
  async refreshToken(userPayload: any) {
    // Get fresh tokens using the payload validated in JwtRefreshStrategy
    const tenant = await this.tenantRepository.findTenantByCode(userPayload.tenantCode);
    if (!tenant || tenant.status !== 'active') {
      throw new UnauthorizedException('Tenant is inactive or invalid');
    }

    const tokens = await this.authService.generateTokens({
      sub: userPayload.userId,
      tenantId: userPayload.tenantId,
      tenantCode: userPayload.tenantCode,
      schemaName: userPayload.schemaName,
  ...
  async getProfile(userId: string, tenantCode: string) {
    const tenant = await this.tenantRepository.findTenantByCode(tenantCode);
    if (!tenant) {
      throw new NotFoundException('Tenant not found');
    }

    const userRecord = await this.tenantRepository.findUserById(userId);
    if (!userRecord) {
  ...
  async update(tenantId: string, dto: UpdateTenantDto) {
    const updated = await this.tenantRepository.updateTenant(tenantId, dto);
    if (!updated) {
      throw new NotFoundException('Tenant not found');
    }
    return updated;
  ...
  async findByCode(tenantCode: string) {
    return this.tenantRepository.findTenantByCode(tenantCode);
  }
}
